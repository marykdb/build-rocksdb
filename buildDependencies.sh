...
if [[ "$OUTPUT_DIR" == *linux_x86_64* ]]; then
  ...
  if (( local_mingw_uses_clang )); then
    ...
    # GCC 9.2 bundled with Kotlin/Native crashes on AVX-512 intrinsics when
    # building dependency sources like bzip2. Disable AVX-512 to avoid the
    # internal compiler error until the toolchain is updated.
    build_common::append_unique_flag EXTRA_CFLAGS "-mno-avx512f"
    build_common::append_unique_flag EXTRA_CXXFLAGS "-mno-avx512f"
  fi
